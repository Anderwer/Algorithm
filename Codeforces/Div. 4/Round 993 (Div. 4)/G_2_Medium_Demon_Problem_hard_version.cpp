...
int dfs(vector<vector<int>>& g, vector<bool>& vis, int x)
{
    if(vis[x] == true) return 0;
    int res = 1;
    vis[x] = true;
    for(auto v : g[x]) res += dfs(g, vis, v);
    return res;
}
...
void solve()
{
    ...
    for(int i = 0; i < n; i++) g[a[i]].push_back(i); //反向建边
    
    int ans = 0;
    for(int i = 0; i < n; i++)
    {
        if(fg.pointOnLoop[i] == false && a[i] == fg.jointPointOnLoop[i]) ans = max(ans, dfs(g, vis, i));
    }
    cout << ans + 2 << "\n";
}
