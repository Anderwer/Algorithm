...
struct DeleteEdgeGraph
{
    ...
    void DSPP() //Delete Shortest Path Problem
    {
        dijkstra(n, distn, 0);
...
void solve()
{
    ...
    while(q--)
    {
        ...
        if(D.ID[i] == -1) //不在路径上
        {
            ans = min(ans, D.dist1[u] + val + D.distn[v]);
            ans = min(ans, D.dist1[v] + val + D.distn[u]);
        }
        else
        {
            ans = ans + val - w;
            if(val > w) ans = min(ans, D.query(1, 1, D.cnt, D.ID[i]));
        }
        cout << ans << "\n";
    }
}
